...
PHP_FUNCTION(opentelemetry_start_cli_tracer) {
    ...
    if (zend_parse_parameters(
            ZEND_NUM_ARGS() TSRMLS_CC, "|ssl", &traceparent, &traceparent_len, &baggage, &baggage_len, &kind) ==
        FAILURE) {
        RETURN_FALSE;
    }
...
PHP_FUNCTION(opentelemetry_get_traceparent) {
    if (is_has_provider()) {
        std::string traceparent =
            OPENTELEMETRY_G(provider)->formatTraceParentHeader(OPENTELEMETRY_G(provider)->firstOneSpan());
...
PHP_FUNCTION(opentelemetry_add_event) {
    if (is_has_provider()) {
        ...
        if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sa", &name, &name_len, &attributes) == FAILURE) {
            RETURN_FALSE;
        }
