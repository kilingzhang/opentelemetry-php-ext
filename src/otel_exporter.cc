...
void OtelExporter::initUDP(const char *ip, int port) {
    ...
    debug("sock_fd open success.");
    this->resolveUDPAddr(3);
    auto *pTimer = new CTimer("dns_lookup");
    pTimer->AsyncLoop(OPENTELEMETRY_G(udp_look_up_time) * 1000,
                      [this]() -> void { this->resolveUDPAddr(3); });  // 异步循环执行，间隔时间10毫秒
}

void OtelExporter::resolveUDPAddr(int reTry) {
    if (reTry <= 0) {
        log("resolveUDPAddr retry max times");
        return;
    }
    reTry--;
    ...
    debug("look up dns : " + std::string(this->addr_ip));
    ...
    debug("resolveUDPAddr ip : " +
          std::string(inet_ntop(AF_INET, &this->addr_in->sin_addr, buf, INET_ADDRSTRLEN)));
}
...
void OtelExporter::sendTracerByUDP(const std::string &data) {
    if (this->sock_fd > 0 && this->addr_in != nullptr && !data.empty()) {
        ...
        if (dataSize <= maxUdpSize) {
            ssize_t nums = sendto(
                this->sock_fd, data.c_str(), data.size(), 0, (struct sockaddr *) this->addr_in, sizeof(*this->addr_in));
            char buf[INET_ADDRSTRLEN];
            if (nums < 0) {
                log("sendto message failed, ip:" +
                    std::string(inet_ntop(AF_INET, &this->addr_in->sin_addr, buf, INET_ADDRSTRLEN)) +
                    ", size:" + std::to_string(data.size()) + ", error:" + strerror(errno));
            } else {
                debug("sendto message success, ip:" +
                      std::string(inet_ntop(AF_INET, &this->addr_in->sin_addr, buf, INET_ADDRSTRLEN)) +
            ...
            for (int i = 0; i < shards; i++) {
                // 协议 md5:9,0:end:数据
                auto warp = id + ":" + std::to_string(shards) + "," + std::to_string(i) +
                            ":end:" + data.substr(i * maxUdpSize, maxUdpSize);
                // 防止丢包 多发一次
                sendto(this->sock_fd,
                       warp.c_str(),
                       warp.size(),
                       ...
                       sizeof(*this->addr_in));
                ssize_t nums = sendto(this->sock_fd,
                                      warp.c_str(),
                                      warp.size(),
                                      0,
                                      (struct sockaddr *) this->addr_in,
...
void OtelExporter::sendTracer(ExportTraceServiceRequest *request, long long int milliseconds) {
    ...
    if (!status.ok()) {
        debug("Export() failed: " + status.error_message() + " trace id : " +
              traceId(request->resource_spans().Get(0).instrumentation_library_spans().Get(0).spans().Get(0)));
        return;
    }
}
...
void OtelExporter::sendAsyncTracer(ExportTraceServiceRequest *request, long long int milliseconds) {
    ...
    call->response_reader = stub_->PrepareAsyncExport(&call->context, *request, &cq_);
...
void OtelExporter::AsyncCompleteRpc() {
    ...
    while (this->cq_.Next(&got_tag, &ok)) {
        ...
        if (!call->status.ok()) {
            debug("Export() failed: " + call->status.error_message());
        }
