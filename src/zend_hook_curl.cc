...
void opentelemetry_curl_exec_handler(INTERNAL_FUNCTION_PARAMETERS) {
    ...
    if (is_record) {
        ...
        span = OPENTELEMETRY_G(provider)->createSpan(php_url_path == nullptr ? "/" : php_url_path,
        ...
        if (php_url_path) {
            target = std::string(php_url_path);
        }

        if (php_url_query && !is_contain("", php_url_query)) {
            target = target + "?" + std::string(php_url_query);
        }

        set_string_attribute(span->add_attributes(), "http.target", target);
    ...
    if (is_record == 1) {
        ...
        zval *response_http_code, *response_primary_ip, *response_total_time, *response_name_lookup_time,
            *response_connect_time, *response_pre_transfer_time, *response_start_transfer_time, *response_redirect_time;
        ...
        set_double_attribute(
            span->add_attributes(), "http.response_name_lookup_time", Z_DVAL_P(response_name_lookup_time));
        // 建立连接时间
        ...
        set_double_attribute(
            span->add_attributes(), "http.response_pre_transfer_time", Z_DVAL_P(response_pre_transfer_time));
        // 开始传输
        response_start_transfer_time = zend_hash_str_find(Z_ARRVAL(url_response), ZEND_STRL("starttransfer_time"));
        set_double_attribute(
            span->add_attributes(), "http.response_start_transfer_time", Z_DVAL_P(response_start_transfer_time));
        // 重定向时间
