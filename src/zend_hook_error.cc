...
extern void (*opentelemetry_original_zend_error_cb)(
    int type, const char *error_filename, const uint error_lineno, const char *format, va_list args);

void (*opentelemetry_original_zend_error_cb)(
    int type, const char *error_filename, const uint error_lineno, const char *format, va_list args) = nullptr;

void opentelemetry_error_cb(
    int type, const char *error_filename, const uint error_lineno, const char *format, va_list args) {
#else

extern void (*opentelemetry_original_zend_error_cb)(int type,
                                                    const char *error_filename,
                                                    const uint32_t error_lineno,
                                                    zend_string *message);

void (*opentelemetry_original_zend_error_cb)(int type,
                                             const char *error_filename,
                                             const uint32_t error_lineno,
...
void opentelemetry_error_cb(int type, const char *error_filename, const uint32_t error_lineno, zend_string *message) {
    ...
    case E_RECOVERABLE_ERROR:
        level = "PHP_ERROR_FATAL";
        break;
    case E_WARNING:
    case E_CORE_WARNING:
    case E_COMPILE_WARNING:
    case E_USER_WARNING:
        level = "PHP_ERROR_WARNING";
        break;
    case E_NOTICE:
    case E_USER_NOTICE:
    case E_STRICT:
    ...
    case E_USER_DEPRECATED:
        level = "PHP_ERROR_NOTICE";
        break;
    default:
        level = "PHP_ERROR_" + std::to_string(type);
        break;
    }
    ...
    if (is_equal("E_NONE", OPENTELEMETRY_G(error_level))) {
        isCollector = false;
    } else if (is_equal("E_ERROR", OPENTELEMETRY_G(error_level)) &&
               (level == "PHP_ERROR_WARNING" || level == "PHP_ERROR_NOTICE")) {
        isCollector = false;
    ...
    if (isCollector && is_has_provider()) {
        ...
        if (function_name.empty()) {
            function_name = "unknow_function_name";
        }
