...
std::vector<std::string> explode(const std::string &delimiter, const std::string &str) {
    ...
    while (i < length) {
        int j = 0;
        while (i + j < length && j < delLength && str[i + j] == delimiter[j]) j++;
...
std::string find_code_stacktrace(zend_execute_data *caller) {
    ...
    if (caller->func && caller->func->common.function_name) {
    ...
    } else if (caller->func && caller->func->internal_function.function_name) {
