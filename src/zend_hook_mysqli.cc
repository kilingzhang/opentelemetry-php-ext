...
void opentelemetry_mysqli_handler(INTERNAL_FUNCTION_PARAMETERS) {
    ...
    if (!opentelemetry_mysqli_original_handler_map.empty() &&
        opentelemetry_mysqli_original_handler_map.find(cmd) != opentelemetry_mysqli_original_handler_map.end()) {
...
void register_zend_hook_mysqli() {
    mysqliKeysCommands = {"mysqli_affected_rows",
                          "mysqli_autocommit",
                          "mysqli_change_user",
    ...
    for (const auto &item : mysqliKeysCommands) {
        if ((old_function = OPENTELEMETRY_OLD_FN_TABLE(CG(function_table), item.c_str())) != nullptr) {
